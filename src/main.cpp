...
struct ChooseSortLayer : public geode::Popup<LevelListLayer*>
{
	...
	void sortList(Sorter* sorter)
	{
		...
		std::sort(newOrder.begin(), newOrder.end(), [&](GJGameLevel* a, GJGameLevel* b) { return sorter->operator_lt(a, b); });
	...
	void onApply(CCObject*)
	{
		...
		//i cant find out how robtop updates but this works the same way
	...
	void setupPreview(CCArrayExt<GJGameLevel*> levels)
	{
		...
		for(int i = 0; i < rows; i++)
		{
			for(int j = columns; j >= 0; j--)
			{
				...
				auto diffSprite = GJDifficultySprite::create(getDiffSpriteAverage(level), static_cast<GJDifficultyName>(0));
				diffSprite->updateFeatureState(getFeatureState(getRating(level)));
	...
	bool setup(LevelListLayer* list) override
	{
	...
	static ChooseSortLayer* create(LevelListLayer* listlayer) {
		...
		if (ret->initAnchored(350, 280, listlayer, "GJ_square01.png")) {
			ret->autorelease();
			return ret;
		}
		delete ret;
...
struct MyListLayer : geode::Modify<MyListLayer, LevelListLayer>
{
	...
	{
		ChooseSortLayer::create(this)->show();
	}
	$override
	bool init(GJLevelList* list)
	{
		...
		if(list->m_listType != GJLevelType::Editor) return true;
		...
		if(auto node = getChildByID("right-side-menu"))
		{
			...
			auto btn = CCMenuItemSpriteExtra::create(
				spr,
				this,
				menu_selector(MyListLayer::onSort));
